...
int main()
{
  ...
  while(1)
  {
    ...
    else if(strcmp(command, "INSERT") == 0)
    {
      ...
      song* song_temp = makeSong(" "," ",0);
      song_temp->artist = songs[idx]->artist;
      song_temp->duration = songs[idx]->duration;
      song_temp->name = songs[idx]->name;
      song_temp->next = NULL;

      if(addSongToQueue(mp, song_temp) == 1)
